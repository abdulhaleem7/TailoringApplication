...
async function GetAllOrders(){
    const getOrders = await fetch(`https://localhost:7060/api/Cart/GetProccessingCart`);
...
let DisplayOrders = async() =>{
    ...
    for(var dat of gets.data)
    {
        ...
        <td><button id="${dat.id}" class="btn btn-success">done</button></td>
        </tr>
        `
    }
    getmeasuremnet();
...
const redirect1 =()=>{
    ...
    clicking.forEach(x => {
         x.addEventListener("click",(f)=>{
             ...
             window.location.href =`/template/pages/tables/GetproccessingOrder.html?id=${f.target.id}`
         })
     })
