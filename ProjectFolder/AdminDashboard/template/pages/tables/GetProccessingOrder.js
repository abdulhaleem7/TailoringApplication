...
async function GetOrderByCartId() {
    const getOrders = await fetch(`https://localhost:7060/GetProccessingOrderByCartId/${id}`)
...
let displayDetails = async ()=>{
    ...
    for(var x of get.data)
    {
        ...
        <td>
            <div class="progress">
            <div class="progress-bar bg-warning" role="progressbar" style="width: ${x.dateDiff*10}%" aria-valuenow="${x.dateDiff*10}" aria-valuemin="0" aria-valuemax="100">${x.dateDiff*10}</div>
            </div>
        </td>
        <td>
            #${x.totalPrice}
